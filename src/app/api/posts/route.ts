...
const cursorCache = new Map<
  string,
  { cursor: string | undefined; timestamp: number }
>()

// 전체 포스트 수 캐시
// key: "search:category:tag" -> value: { totalPosts: number, timestamp: number }
const totalPostsCache = new Map<
  string,
  { totalPosts: number; timestamp: number }
>()
...
// 전체 포스트 수 캐시 키 생성 함수
function getTotalPostsCacheKey(
  search: string,
  category: string,
  tag: string,
): string {
  return `total:${search}:${category || 'all'}:${tag || 'none'}`
}

// 전체 포스트 수 계산 함수
async function calculateTotalPosts(
  search: string,
  category: string | undefined,
  tag: string | undefined,
  limit: number,
): Promise<number> {
  const cacheKey = getTotalPostsCacheKey(search, category || '', tag || '')
  const cached = totalPostsCache.get(cacheKey)

  if (cached && Date.now() - cached.timestamp < CACHE_TTL) {
    return cached.totalPosts
  }
  ...
  while (hasMore) {
    ...
    if (tag) {
      ...
      response = await getPublishedPosts(limit, cursor, search, category)
    }
...
function cleanExpiredCache() {
  ...
  for (const [key, value] of totalPostsCache.entries()) {
    if (now - value.timestamp > CACHE_TTL) {
      totalPostsCache.delete(key)
    }
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (page > 1) {
      ...
      if (cachedCursor && Date.now() - cachedCursor.timestamp < CACHE_TTL) {
        ...
        for (let i = 1; i < page; i++) {
          const tempCacheKey = getCacheKey(
            searchQuery,
            category || '',
            tag || '',
            i,
          ...
          if (tempCached && Date.now() - tempCached.timestamp < CACHE_TTL) {
            ...
            if (tag) {
              ...
              tempResponse = await getPublishedPosts(
                limit,
                tempCursor,
                searchQuery,
                category,
    ...
    // 전체 포스트 수 계산 (캐시 사용)
    const totalPosts = await calculateTotalPosts(
      searchQuery,
      category,
      tag,
      limit,
